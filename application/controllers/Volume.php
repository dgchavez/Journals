...
class Volume extends CI_Controller {
    ...
    public function db_Volumes() {
        // Check if an archive toggle request is made
        if ($this->input->post('toggle_archive')) {
            // Extract the volume ID and new archive status from the request
            $volumeid = $this->input->post('volumeid');
            $new_status = $this->input->post('isArchive') == 0 ? 1 : 0; // Toggle the archive status
    
            // Call the toggle_archive function to update the status
            $this->toggle_archive($volumeid, $new_status);
        }
    ...
    public function toggle_archive($volumeid, $isArchive) {
        // Update the volume's archive status in the database
        $this->db->where('volumeid', $volumeid);
        $this->db->update('Volume', ['isArchive' => $isArchive]);
    
        // Redirect back to the list of volumes
        redirect('volume/db_Volumes');
    }
